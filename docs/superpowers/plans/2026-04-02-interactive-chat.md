...
interface SelectableCardGroupProps {
    items: Array<{ id: string; label: string; node: (selected: boolean, onClick: () => void) => ReactNode }>;
...
export function SelectableCardGroup({
    ...
    const [selectedId, setSelectedId] = useState<string | null>(confirmedId ?? null);
            ...
            <div className={styles.cards}>
                {items.map((item) => (
                    <div key={item.id}>
                        {item.node(
                            selectedId === item.id,
                            () => !isLocked && setSelectedId(item.id === selectedId ? null : item.id),
                        )}
                    </div>
                ))}
            </div>
...
export function parseQuickReplies(text: string): string[] | null {
    ...
    // Yes/No patterns
    if (
        /(?:would you like|shall i|should i|do you want|want me to|ready to)\b/.test(lower)
    ) {
...
```typescript
const [toolResults, setToolResults] = useState<Record<string, { tool_name: string; result: unknown }>>({});
```
        ...
        {Object.entries(toolResults).map(([toolId, { tool_name, result }]) => {
            if (tool_name === 'search_flights' && Array.isArray(result)) {
                return (
                    <div key={toolId} className={styles.resultCards}>
                        <SelectableCardGroup
                            items={(result as Array<Record<string, unknown>>).map((f, i) => ({
                                id: String(i),
            ...
            if (tool_name === 'search_hotels' && Array.isArray(result)) {
                return (
                    <div key={toolId} className={styles.resultCards}>
                        <SelectableCardGroup
                            items={(result as Array<Record<string, unknown>>).map((h, i) => ({
                                id: String(i),
            ...
            if (tool_name === 'search_experiences' && Array.isArray(result)) {
                return (
                    <div key={toolId} className={styles.resultCards}>
                        <SelectableCardGroup
                            items={(result as Array<Record<string, unknown>>).map((e, i) => ({
                                id: String(i),
...
{!isSending && allMessages.length > 0 && allMessages[allMessages.length - 1].role === 'assistant' && (() => {
    const chips = parseQuickReplies(allMessages[allMessages.length - 1].content);
    return chips ? (
        <QuickReplyChips
            chips={chips}
...
export function MapPreviewCard({ latitude, longitude, name }: MapPreviewCardProps) {
 ...
 */
export function parseItinerary(text: string): { before: string; days: DayPlan[]; after: string } | null {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        } else if (currentDay && (line.startsWith('•') || line.startsWith('-') || line.startsWith('*'))) {
    ...
    const after = lines.slice(itineraryEnd + 1).join('\n').trimStart();
...
```tsx
const itineraryData = msg.role === 'assistant' ? parseItinerary(msg.content) : null;
```
