                ...
                <div key={field.type} className={styles.field}>
                    ...
                    {field.type === "departure_date" && (
                        <input
                            ...
                            onChange={(e) =>
                                set("departure_date", e.target.value)
                            }
                            disabled={disabled || isLocked}
                            readOnly={isLocked}
                        />
...
export function parseTripFormFields(content: string): {
    ...
    for (const line of listLines) {
        ...
        if (
            /travel(ing|l?)\s+from|origin|departure\s+(city|airport)/.test(
                lower,
            )
        ) {
            fields.push({ type: "origin", label: "Origin city" });
        } else if (
            /travel\s+dates|departure.*return|when.*travel/.test(lower)
        ) {
            ...
            /traveler|passenger|how\s+many\s+(people|guest)/.test(lower)
        ) {
