...
describe("conversations repository", () => {
  ...
  describe("getOrCreateConversation", () => {
    it("returns existing conversation for a trip", async () => {
      const existing = { id: "conv-1", trip_id: "trip-1", created_at: "2026-01-01" };
      vi.mocked(query).mockResolvedValueOnce({ rows: [existing], rowCount: 1 } as never);
    ...
    it("creates new conversation if none exists", async () => {
      const created = { id: "conv-new", trip_id: "trip-2", created_at: "2026-01-01" };
      vi.mocked(query).mockResolvedValueOnce({ rows: [created], rowCount: 1 } as never);
  ...
  describe("insertMessage", () => {
    it("inserts a user message", async () => {
      ...
      vi.mocked(query).mockResolvedValueOnce({ rows: [msg], rowCount: 1 } as never);
  ...
  describe("getMessagesByConversation", () => {
    it("returns messages ordered by created_at", async () => {
      const messages = [
        ...
        { id: "msg-2", role: "assistant", content: "Hello!", created_at: "2026-01-01T00:01:00Z" },
      ];
      vi.mocked(query).mockResolvedValueOnce({ rows: messages, rowCount: 2 } as never);

      const result = await getMessagesByConversation("conv-1");

      expect(result).toHaveLength(2);
      expect(query).toHaveBeenCalledWith(expect.stringContaining("ORDER BY"), ["conv-1"]);
    });
