  ...
  describe("POST /trips (createTrip)", () => {
    ...
    it("returns 201 with created trip", async () => {
      ...
      expect(tripRepo.createTrip).toHaveBeenCalledWith(
        userId,
        expect.objectContaining({ destination: "Barcelona", budget_total: 3000 }),
      );
    });
    ...
    it("returns 500 on repo error", async () => {
      vi.mocked(tripRepo.createTrip).mockRejectedValueOnce(new Error("DB error"));

      const res = await request(app).post("/trips").send({ destination: "Barcelona" });
