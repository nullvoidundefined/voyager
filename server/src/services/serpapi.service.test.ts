...
describe("serpapi.service", () => {
  ...
  describe("serpApiGet", () => {
    ...
    it("returns parsed JSON on success", async () => {
      ...
      const result = await serpApiService.serpApiGet("google_hotels", { q: "hotels in Paris" });
    ...
    it("throws on API error", async () => {
      ...
      await expect(serpApiService.serpApiGet("google_flights", {})).rejects.toThrow(
        "SerpApi error: 401",
      );
    });
    ...
    it("throws when SERPAPI_API_KEY is not set", async () => {
      ...
      await expect(freshModule.serpApiGet("google_flights", {})).rejects.toThrow(
